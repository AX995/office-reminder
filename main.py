...
from PySide6.QtWidgets import (
    ...
    QDialog, QFormLayout, QLineEdit, QTabWidget, QStyle, QSizePolicy
)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.tab_lists = {}          # tab_name → QListWidget
        self.tab_counts = {}         # tab_name → QLabel（数量角标暂用标签文字表示）
        ...
        for btn in [add_btn, import_btn, edit_btn, open_btn, done_btn, del_btn]:
    ...
    def refresh_all_tabs(self):
        ...
        for tab_name, lst in self.tab_lists.items():
            ...
            for t in tasks:
                ...
                item.setData(Qt.UserRole, t)  # 存储完整 task 数据

                if is_done:
                    item.setForeground(Qt.gray)
                elif not path_ok:
                    item.setForeground(Qt.red)
                    item.setText(f"❌ {t['name']}（路径失效）")
    ...
    def check_reminders(self):
        ...
        for t in self.mgr.tasks:
            ...
            if t["type"] == "每日任务":
                ...
                if now.weekday() == 0 and t["remind_time"] == current_time_str and t.get("last_reminded") != key:
                    should_remind = True
            elif t["type"] == "每月固定日任务":
